...
function activate(context) {
	...
	let disposable = vscode.commands.registerCommand('addruby.narou', function () {
		...
		if(url.test(text)==true){	// URLの場合、専用形式に置き換える
			...
			if(ruby.test(text)==true){	// ルビ振り済み（読みなし）の場合、除去
				let re_rubied = new RegExp('(?<=\\x7c).*(?=《》)')
				let rubied = text.match(re_rubied)[0]
				editor.edit(builder => builder.replace(selection, rubied)) // 選択してた文字列を、newTextに置き換え（カッコを追加）
			}else{
				let newText = ("|"+text+"《》")
